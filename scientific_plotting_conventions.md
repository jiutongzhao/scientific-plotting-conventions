...
A practical reference for making figures that are **publication-ready**, **legible**, and
**editable** — and how to enforce every rule with **matplotlib**.

> **The one rule that explains all the others:** *Build the figure at the exact physical
> size it will be printed or projected.* Never draw a big figure and shrink it in Word/LaTeX/
> PowerPoint — scaling silently shrinks your fonts and line weights below the legibility
> limits and makes panels inconsistent. Set the size in **inches/mm**, set the font in
> **points**, and what you see is what the reader gets.
...
![To-scale page layouts for six publishers: paper size and margins, full-width and part-width figures at each journal's true column width and a distinct aspect ratio, multi-panel labels and figure captions in each house style, greeked body text, and a three-typeface specimen](publisher_page_layouts.png)

*This schematic is generated by [`publisher_page_layouts.py`](publisher_page_layouts.py); the matplotlib techniques it uses — column widths (§11.4), panel labels (§11.5), editable vector export (§5) — are all explained in this document.*
